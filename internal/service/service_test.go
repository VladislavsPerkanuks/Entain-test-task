...
func (m *MockRepository) UpdateUserBalance(userID int, delta decimal.Decimal) error {
...
func (m *MockRepository) GetTransactionByID(id uuid.UUID) (*model.Transaction, error) {
	args := m.Called(id)
	tx, _ := args.Get(0).(*model.Transaction)
	return tx, args.Error(1)
}

func (m *MockRepository) InsertTransaction(tx *model.Transaction) error {
	args := m.Called(tx)
	return args.Error(0)
}
...
func TestProcessTransaction(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "win",
			tx:   &model.Transaction{ID: winID, UserID: 1, State: model.TransactionStateWin, Amount: decimal.NewFromInt(100)},
			setupMock: func(m *MockRepository) {
		...
		{
			name: "lose",
			tx:   &model.Transaction{ID: loseID, UserID: 2, State: model.TransactionStateLose, Amount: decimal.NewFromInt(50)},
			setupMock: func(m *MockRepository) {
				m.On("WithDBTransaction", mock.Anything).Return(nil)
		...
		{
			name: "update error",
			tx:   &model.Transaction{ID: uuid.New(), UserID: 1, State: model.TransactionStateLose, Amount: decimal.NewFromInt(5)},
			setupMock: func(m *MockRepository) {
				m.On("WithDBTransaction", mock.Anything).Return(nil)
				m.On("GetTransactionByID", mock.Anything).Return(nil, repository.ErrTransactionNotFound)
				m.On("InsertTransaction", mock.Anything).Return(nil)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := svc.ProcessTransaction(tt.tx)
...
func TestGetBalance(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.setupMock(repo)
			svc := NewTransactionService(repo)
			balance, err := svc.GetBalance(tt.userID)
